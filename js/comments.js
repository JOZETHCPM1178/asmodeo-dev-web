    ...
    <div class="cmt ${c.pinned ? 'pinned' : ''} ${depth > 0 ? 'reply' : ''}">
      ...
      <div class="cmt-body">
        ...
        <div class="cmt-acts">
          ...
          ${depth === 0 ? `<button class="cact" onclick="toggleReply('${c.id}')">${(window._t&&window._t['reply'])||''}</button>` : ''}
          ${u?.isAdmin ? `
  ...
  const newCmtForm = u
    ? `<div class="new-cmt">
        ...
        <div class="cmt-inp-wrap">
          <textarea class="txta" id="new-cmt-txt" rows="3" placeholder="${(window._t&&window._t['comment_placeholder'])||''}"></textarea>
          <button class="btn btn-primary btn-sm" style="margin-top:7px" onclick="sendCmt('${postId}')">${(window._t&&window._t['publish_comment'])||''}</button>
        </div>
      </div>`
    : `<div class="login-prompt">${(window._t&&window._t['login_to_comment'])||''} · <a style="color:var(--p);cursor:pointer" onclick="showLogin()">Entrar</a></div>`;
