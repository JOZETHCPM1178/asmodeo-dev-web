...
function showReportModal(type, id, name = '') {
  ...
  modal.innerHTML = `
    <div style="background:var(--s1);border-radius:16px;padding:24px;max-width:400px;width:100%;border:1px solid var(--p)">
      <div style="font-family:var(--font1);font-size:1rem;font-weight:700;margin-bottom:16px">🚩 Reportar ${type === 'post' ? 'publicación' : type === 'comment' ? 'comentario' : 'perfil'}${name ? ': '+name : ''}</div>
      <div style="font-size:.82rem;color:var(--t3);margin-bottom:14px">¿Por qué quieres reportar este contenido?</div>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
        ${reasons.map(r => `<button class="btn btn-ghost btn-sm" style="justify-content:flex-start;text-align:left" onclick="submitReport('${type}','${id}','${r}')">${r}</button>`).join('')}
      </div>
      <button class="btn btn-ghost btn-sm" style="width:100%;justify-content:center" onclick="document.getElementById('report-modal').remove()">Cancelar</button>
    </div>`;
