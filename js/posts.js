  ...
  setMain(`
    <section id="hero">
      ...
      <div class="hero-inner fu">
        ...
        <div class="hero-btns">
          <button class="btn btn-ghost" onclick="showCat('apk')">${(window._t&&window._t['btn_apk'])||''}</button>
...
async function showPost(id) {
  ...
  const intentar = async () => {
    ...
    try {
          ...
          <div class="detail-body">
            <div>
              ...
              ${p.downloadLink ? `
                ...
                <button class="vt-btn" onclick="abrirVirusTotal('${p.downloadLink.replace(/'/g,"\\'")}')">
              ...
              <button class="btn btn-ghost" style="width:100%;justify-content:center;color:#ef4444;margin-top:6px" data-reportid="${id}" data-reporttitle="${(p.title||'').replace(/"/g,'&quot;')}" id="report-post-btn-${id}">🚩 Reportar publicación</button>
            ...
            <div>
              <div class="sidebar-card">
                ...
                ${p.version ? `<div class="s-row"><span class="key">Versión</span><span style="font-family:var(--font1)">v${p.version}</span></div>` : ''}
                ${p.stability ? `<div class="s-row"><span class="key">Estado</span><span class="badge ${p.stability === 'beta' ? 'b-apk' : p.stability === 'experimental' ? '' : 'b-admin'}" style="${p.stability === 'experimental' ? 'background:#ff6b00;color:#fff' : ''}">${p.stability === 'beta' ? '🧪 Beta' : p.stability === 'experimental' ? '⚠️ Experimental' : '✅ Estable'}</span></div>` : ''}
      ...
      setTimeout(() => {
        const rBtn = document.getElementById('report-post-btn-' + id);
        if (rBtn) {
          rBtn.onclick = () => window.showReportModal('post', rBtn.dataset.reportid, rBtn.dataset.reporttitle);
        }
      }, 300);
    } catch (e) {
      if (n < 5) {
