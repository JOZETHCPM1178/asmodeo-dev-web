...
function cargarHomePosts() {
  ...
  const intentar = async () => {
    ...
    try {
      ...
      grid.innerHTML = `
        <div class="carousel-wrap" id="home-carousel">
          <div class="carousel-track" id="carousel-track">
            ${posts.map((p, i) => `<div class="carousel-item">${postCardHTML(p, i)}</div>`).join('')}
          </div>
          <div class="carousel-dots" id="carousel-dots">
            ${Array.from({length: Math.ceil(posts.length/2)}, (_,i) => `<button class="cdot ${i===0?'active':''}" onclick="goToSlide(${i})"></button>`).join('')}
          </div>
        </div>`;
      initCarousel();
    } catch (e) {
