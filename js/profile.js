...
async function showUserProfile(uid) {
  ...
  try {
    ...
    // Seguidores reales + base personalizada
    const fakeBase = parseInt(u.fakeFollowers || 0);
    const isAdmin = u.role === 'admin' || u.email === window.ADMIN_EMAIL;
    const adminBase = isAdmin && fakeBase === 0 ? 10000 : 0;
    const followerCount = realFollowers + fakeBase + adminBase;
        ...
        <div class="prof-pub-card">
          ...
          <div class="prof-pub-body">
            ...
            <div class="prof-pub-actions" style="display:flex;gap:8px;flex-wrap:wrap">
              ...
              ${!isMe && window._currentUser ? `<button class="btn btn-ghost btn-sm" onclick="showReportModal('profile','${uid}','${u.username||u.displayName||'Usuario'}')" style="color:#ef4444">🚩 Reportar</button>` : ''}
            </div>
          </div>
        </div>
...
async function toggleFollow(targetUid) {
  ...
  if (isFollowing) {
    ...
    const { serverTimestamp, collection, addDoc } = window._fb;
