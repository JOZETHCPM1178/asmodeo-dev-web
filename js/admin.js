...
async function renderAdminStats() {
  ...
  try {
    ...
    // Detectar usuarios sospechosos (sin foto, sin bio, creados recientemente)
    const { db, collection, getDocs, query, orderBy, limit } = window._fb;
    const usersSnap = await getDocs(collection(db, 'users'));
    const users = usersSnap.docs.map(d => ({ id: d.id, ...d.data() }));
    const suspicious = users.filter(u => {
      const age = Date.now() - (u.createdAt?.seconds || 0) * 1000;
      return !u.photoURL && !u.bio && age < 86400000 * 3; // Sin foto, sin bio, menos de 3 días
    }).slice(0, 10);

    el.innerHTML = `
      <h3 style="font-family:var(--font1);font-size:.95rem;margin-bottom:16px">📊 Estadísticas</h3>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:24px">
        <div style="background:var(--s1);border-radius:12px;padding:16px;text-align:center;border:1px solid var(--border)">
          <div style="font-size:1.6rem;font-family:var(--font1);color:var(--p)">${fmtN(stats.totalVisits)}</div>
          <div style="font-size:.75rem;color:var(--t3)">Visitas totales</div>
        </div>
        <div style="background:var(--s1);border-radius:12px;padding:16px;text-align:center;border:1px solid var(--border)">
          <div style="font-size:1.6rem;font-family:var(--font1);color:#10b981">${fmtN(stats.totalDownloads)}</div>
          <div style="font-size:.75rem;color:var(--t3)">Descargas totales</div>
        </div>
        <div style="background:var(--s1);border-radius:12px;padding:16px;text-align:center;border:1px solid var(--border)">
          <div style="font-size:1.6rem;font-family:var(--font1);color:#f59e0b">${fmtN(stats.totalUsers)}</div>
          <div style="font-size:.75rem;color:var(--t3)">Usuarios registrados</div>
        </div>
        <div style="background:var(--s1);border-radius:12px;padding:16px;text-align:center;border:1px solid var(--border)">
          <div style="font-size:1.6rem;font-family:var(--font1);color:#8b5cf6">${fmtN(stats.totalPosts)}</div>
          <div style="font-size:.75rem;color:var(--t3)">Publicaciones</div>
        </div>
      </div>
      ${suspicious.length > 0 ? `
        <h3 style="font-family:var(--font1);font-size:.9rem;margin-bottom:12px;color:#ef4444">⚠️ Usuarios sospechosos (${suspicious.length})</h3>
        ${suspicious.map(u => `
          <div class="post-row" style="align-items:center;gap:12px">
            <img src="${u.photoURL || avatarUrl(u.displayName)}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;flex-shrink:0"/>
            <div style="flex:1;min-width:0">
              <div style="font-size:.85rem;font-weight:700">${u.username || u.displayName || 'Sin nombre'}</div>
              <div style="font-size:.72rem;color:var(--t3)">${u.email} · Sin foto · Sin bio</div>
            </div>
            <button class="btn btn-ghost btn-sm" style="color:#ef4444;flex-shrink:0" onclick="banUser('${u.id}',false).then(()=>renderAdminStats())">🔨 Ban</button>
          </div>`).join('')}` : '<div style="text-align:center;color:var(--t3);font-size:.85rem;padding:20px">✅ No se detectaron usuarios sospechosos</div>'}`;
  } catch(e) {
    el.innerHTML = `<div class="empty"><span class="empty-ico">⚠️</span><h3>Error: ${e.message}</h3></div>`;
  }
}
