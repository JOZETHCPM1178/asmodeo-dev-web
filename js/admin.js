      ...
      ${reports.map(r => `
        <div class="post-row" style="flex-direction:column;gap:8px;align-items:flex-start;margin-bottom:12px">
          ...
          <div style="display:flex;gap:6px;width:100%;flex-wrap:wrap">
            ...
            <button class="btn btn-ghost btn-sm" onclick="showReportResponseForm('${r.id}')">📋 Responder</button>
          ...
          <div id="resp-form-${r.id}" style="display:none;width:100%">
            ...
            <div style="display:flex;gap:8px;margin-top:6px">
              <button class="btn btn-primary btn-sm" onclick="sendReportResponse('${r.id}')">📤 Enviar</button>
...
function showReportResponseForm(reportId) {
...
async function sendReportResponse(reportId) {
  ...
  const report = window._adminReports?.[reportId];
  const reportedByUid = report?.reportedBy;
  if (!reportedByUid) return toast('Error: UID del usuario no encontrado', 'err');
  try {
