...
function renderNavAuth() {
  ...
  if (u) {
    ...
    const roleBadge = u.isAdmin
      ? `<span class='badge b-admin'>${u.customRole || 'Admin'}</span>`
      : u.isAdminJr
      ? `<span class='badge b-adminjr'>${u.customRole || 'Admin Jr'}</span>`
      : '';
    el.innerHTML = `
      <div class="av-wrap">
    ...
    el.innerHTML = `
      <div class="auth-btns" style="display:flex;gap:8px">
        <button class="btn btn-ghost btn-sm" onclick="showLogin()">${T.login||'Entrar'}</button>
        <button class="btn btn-primary btn-sm" onclick="showRegister()">${T.register||'Registrarse'}</button>
      </div>`;
    if (mob) mob.innerHTML = `
      <button class="mob-link" onclick="showLogin()">🔑 ${T.login||'Entrar'}</button>
      <button class="mob-link" onclick="showRegister()">✨ ${T.register||'Registrarse'}</button>`;
  }
}
